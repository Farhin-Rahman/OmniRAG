    ...
    try:
        ...
        with fitz.open(file_path) as doc:
            for page_num in range(len(doc)):
                ...
                if not text:
                    logger.info(f"Page {page_num + 1} has no text. Attempting OCR fallback.")
                    try:
                        import easyocr
                        import numpy as np
                        ...
                        if text:
                            logger.info(f"OCR successfully extracted {len(text.split())} words from page {page_num + 1} using EasyOCR")
                        else:
        ...
        if chunks:
            ...
            for i in range(0, len(chunks), batch_size):
                ...
                for chunk in batch:
                    vector = await embedding_service.generate_embedding(chunk["text"], mode="document")
